...
//        try{
//            return getClubStudentList(connection, studentId);
//        } catch (Exception e) {
//            throw new RuntimeException(e);
//        }
...
//    select a.id as student_id, a.name as student_name, c.club_id, c.club_name from jdbc_students a  inner join jdbc_club_registrations b on a.id=b.student_id inner join jdbc_club c on b.club_id=c.club_id order by a.id asc, b.club_id asc

    @Override
    public List<ClubStudent> findClubStudents_left_join(Connection connection) {
        //todo#22 - left join
        String sql = "SELECT a.id AS student_id, a.name AS student_name, c.club_id AS club_id, c.club_name AS club_name FROM jdbc_students a LEFT JOIN jdbc_club_registrations b ON a.id = b.student_id LEFT JOIN jdbc_club c ON b.club_id = c.club_id order by a.id asc, b.club_id asc";
                try{
            return getClubStudentList(connection, sql);
        }catch (Exception e) {
            throw new RuntimeException(e);
